...
unsigned int numRounds(unsigned int keySize) {
    //wenn es 128 Bit-Verschlüsselung ist werden 10 Runden verschlüsselt
    if (keySize == 128) {
        return 10;
    } //bei 192 Bit-Verschlüsselung werden 12 Runden lang verschlüsselt
    else if (keySize == 192) {
        return 12;
    }//bei 256 Bit-Verschlüsselung werden 14 Runden verschlüsselt
    else if (keySize == 256) {
        return 14;
    }
...
uint8_t getSBoxValue(uint8_t num) {
    //den SBox-Wert der eingegebenen Variable zurückbekommen
    return sbox[num];
}
